...
module.exports = function (path) {
	...
	return resolveModulePaths(path)(modulePaths => {
		...
		return deferred.map(modulePaths, modulePath =>
			lstat(resolve(testPath, modulePath))(
				stats => stats.isFile(),
				err => {
				...
				if (isTest) {
					...
					return `\tt.pass(${
						JSON.stringify(`Tests found for '${ modulePath }'`)
					});`;
				}
				return `\tt.fail(${
					JSON.stringify(`Tests missing for '${ modulePath }'`)
				});`;
			})
		)(indexTestsData => {
			const srcCode = [
			...
			return writeFile(
				resolve(path, `${ testIndexName }.js`), srcCode.join("\n")
			);
		});
	});
};
